...
function buildDocument(values) {
  const catalog = indicatorsOf(values.block);
...
function useVISAForm(onSuccess) {
  ...
  useEffect(() => {
    form.setValue("values", valoresZerados(block), {
  ...
  return (
    <div className="grid grid-cols-1 md:grid-cols-2 gap-x-6 gap-y-4">
      {catalog.map(({ key, label, type }) => {
        const fieldError = errors.values?.[key];
            ...
            <Controller
              ...
              render={({ field }) => (
                <input
                  ...
                  inputMode={tipo === TYPE.COUNTING ? "numeric" : "decimal"}
                  placeholder={
                    tipo === TIPO.PROPORCAO
  ...
  return (
    <div className="border border-gray-300 rounded-xl p-8 bg-white shadow-sm overflow-x-auto">
      ...
      <table className="w-full text-sm">
        ...
        <tbody>
          {registers.map((r) => {
            const preenchidos = r.indicators.filter(
              (i) => i.value !== null,
            ).length;
            return (
              <tr
...
function VISAForm({ onSuccess }) {
  const { form, handleSubmit, catalog, filled } =
    useVISAForm(onSuccess);
  const {
    register,
    control,
        ...
        <div>
          ...
          <div className="grid grid-cols-2 gap-3 mt-8">
            <MetricCard
              ...
              value={String(catalogo.length)}
            />
            <MetricCard
              label="Preenchidos"
              value={String(preenchidos)}
              accent
            />
          </div>
        </div>
    ...
    <main className="bg-gray-100 min-h-screen">
      ...
      <div className="w-full relative">
        <h1 className="absolute top-10 bottom-4 left-10 text-3xl text-black font-bold drop-shadow">
          <span className="font-normal mr-1">Vigilancia Sanitaria</span>
        </h1>
      </div>
      {/* hero-ends */}
      ...
      <section className="px-10 py-8">
        <div className="border-b border-gray-300 pb-4">
          ...
          <p className="text-gray-500 text-sm mt-1">
            Indicadores mensais — Vigilância em Saúde (São José/SC)
          </p>
        </div>
      </section>
      {/* page-title-ends */}
